...
// File @finaswap/finavault-sdk/contracts/IBatchFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @finaswap/finavault-sdk/contracts/IFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @finaswap/finavault-sdk/contracts/IStrategy.sol@v1.0.1
// License-Identifier: MIT
...
// File @finaswap/finavault-sdk/contracts/IFinaVaultV1.sol@v1.0.1
// License-Identifier: MIT

interface IFinaVaultV1 {
...
interface ISwapper {
    /// @notice Withdraws 'amountFrom' of token 'from' from the FinaVault account for this swapper.
    /// Swaps it for at least 'amountToMin' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the FinaVault using a plain ERC20 transfer.
    /// Returns the amount of tokens 'to' transferred to FinaVault.
    /// (The FinaVault skim function will be used by the caller to get the swapped funds).
    function swap(
        IERC20 fromToken,
        IERC20 toToken,
        address recipient,
    ...
    /// Withdraws 'amountFrom' of token 'from' from the FinaVault account for this swapper.
    /// Swaps it for exactly 'exactAmountTo' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the FinaVault using a plain ERC20 transfer.
    /// Transfers allocated, but unused 'from' tokens within the FinaVault to 'refundTo' (amountFromMax - amountFrom).
    /// Returns the amount of 'from' tokens withdrawn from FinaVault (amountFrom).
    /// (The FinaVault skim function will be used by the caller to get the swapped funds).
    function swapExact(
        IERC20 fromToken,
        IERC20 toToken,
...
/// @title BankerPair
/// @dev This contract allows contract calls to any contract (except FinaVault)
...
contract BankerPairMediumRiskV1 is ERC20, BoringOwnable, IMasterContract {
    ...
    IFinaVaultV1 public immutable finaVault;
    ...
    Rebase public totalAsset; // elastic = FinaVault shares held by the BankerPair, base = Total fractions held by asset suppliers
    ...
    constructor(IFinaVaultV1 finaVault_) public {
        finaVault = finaVault_;
        masterContract = this;
        feeTo = msg.sender;
    }
    ...
    function accrue() public {
        ...
        uint256 fullAssetAmount = finaVault.toAmount(asset, _totalAsset.elastic, false).add(_totalBorrow.elastic);
    ...
    /// False if tokens from msg.sender in `finaVault` should be transferred.
    function _addTokens(
        IERC20 token,
        uint256 share,
        uint256 total,
        ...
        if (skim) {
            require(share <= finaVault.balanceOf(token, address(this)).sub(total), "BankerPair: Skim too much");
        } else {
            finaVault.transfer(token, msg.sender, address(this), share);
        }
    }
    ...
    function addCollateral(
        ...
        emit LogAddCollateral(skim ? address(finaVault) : msg.sender, to, share);
    }
    ...
    function _removeCollateral(address to, uint256 share) internal {
        ...
        finaVault.transfer(collateral, address(this), to, share);
    }
    ...
    function _addAsset(
        ...
        uint256 allShare = _totalAsset.elastic + finaVault.toShare(asset, totalBorrow.elastic, true);
        ...
        emit LogAddAsset(skim ? address(finaVault) : msg.sender, to, share, fraction);
    }
    ...
    function _removeAsset(address to, uint256 fraction) internal returns (uint256 share) {
        ...
        finaVault.transfer(asset, address(this), to, share);
    }
    ...
    function _borrow(address to, uint256 amount) internal returns (uint256 part, uint256 share) {
        ...
        share = finaVault.toShare(asset, amount, false);
        Rebase memory _totalAsset = totalAsset;
        require(_totalAsset.base >= 1000, "Banker: below minimum");
        _totalAsset.elastic = _totalAsset.elastic.sub(share.to128());
        totalAsset = _totalAsset;
    ...
    function _repay(
        ...
        uint256 share = finaVault.toShare(asset, amount, true);
        uint128 totalShare = totalAsset.elastic;
        _addTokens(asset, share, uint256(totalShare), skim);
        totalAsset.elastic = totalShare.add(share.to128());
        emit LogRepay(skim ? address(finaVault) : msg.sender, to, amount, part);
    }
    ...
    // Function on FinaVault
    uint8 internal constant ACTION_VAULT_DEPOSIT = 20;
    uint8 internal constant ACTION_VAULT_WITHDRAW = 21;
    uint8 internal constant ACTION_VAULT_TRANSFER = 22;
    uint8 internal constant ACTION_VAULT_TRANSFER_MULTIPLE = 23;
    uint8 internal constant ACTION_VAULT_SETAPPROVAL = 24;

    // Any external call (except to FinaVault)
    ...
    /// @dev Helper function for depositing into `finaVault`.
    function _vaultDeposit(
        ...
        return finaVault.deposit{value: value}(token, msg.sender, to, uint256(amount), uint256(share));
    }

    /// @dev Helper function to withdraw from the `finaVault`.
    function _vaultWithdraw(
        ...
        (IERC20 token, address to, int256 amount, int256 share) = abi.decode(data, (IERC20, address, int256, int256));
        return finaVault.withdraw(token, msg.sender, to, _num(amount, value1, value2), _num(share, value1, value2));
    }

    /// @dev Helper function to perform a contract call and eventually extracting revert messages on failure.
    /// Calls to `finaVault` are not allowed for obvious security reasons.
    ...
    function _call(
        ...
        require(callee != address(finaVault) && callee != address(this), "BankerPair: can't call");
    ...
    /// Only applicable to `ACTION_CALL`, `ACTION_VAULT_DEPOSIT`.
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
            ...
            } else if (action == ACTION_VAULT_SETAPPROVAL) {
                (address user, address _masterContract, bool approved, uint8 v, bytes32 r, bytes32 s) =
                    abi.decode(datas[i], (address, address, bool, uint8, bytes32, bytes32));
                finaVault.setMasterContractApproval(user, _masterContract, approved, v, r, s);
            } else if (action == ACTION_VAULT_DEPOSIT) {
                (value1, value2) = _vaultDeposit(datas[i], values[i], value1, value2);
            } else if (action == ACTION_VAULT_WITHDRAW) {
                (value1, value2) = _vaultWithdraw(datas[i], value1, value2);
            } else if (action == ACTION_VAULT_TRANSFER) {
                (IERC20 token, address to, int256 share) = abi.decode(datas[i], (IERC20, address, int256));
                finaVault.transfer(token, msg.sender, to, _num(share, value1, value2));
            } else if (action == ACTION_VAULT_TRANSFER_MULTIPLE) {
                (IERC20 token, address[] memory tos, uint256[] memory shares) = abi.decode(datas[i], (IERC20, address[], uint256[]));
                finaVault.transferMultiple(token, msg.sender, tos, shares);
                ...
                value1 = finaVault.toShare(asset, totalBorrow.toElastic(_num(part, value1, value2), true), true);
    ...
    function liquidate(
        ...
        Rebase memory _totalBorrow = totalBorrow;
        Rebase memory finaVaultTotals = finaVault.totals(collateral);
        ...
        uint256 allBorrowShare = finaVault.toShare(asset, allBorrowAmount, true);
        ...
        if (!open) {
            ...
            finaVault.transfer(collateral, address(this), address(swapper), allCollateralShare);
            swapper.swap(collateral, asset, address(this), allBorrowShare, allCollateralShare);

            uint256 returnedShare = finaVault.balanceOf(asset, address(this)).sub(uint256(totalAsset.elastic));
            ...
            finaVault.transfer(asset, address(this), masterContract.feeTo(), feeShare);
            ...
            finaVault.transfer(collateral, address(this), swapper == ISwapper(0) ? to : address(swapper), allCollateralShare);
            if (swapper != ISwapper(0)) {
                swapper.swap(collateral, asset, msg.sender, allBorrowShare, allCollateralShare);
            }

            finaVault.transfer(asset, msg.sender, address(this), allBorrowShare);
